...
Автоматизация через таск менеджер:

```
cargo make check-and-build
```
...
В Rust существует несколько способов работы с базами данных (БД) в оперативной памяти, от стандартных коллекций до полноценных встраиваемых систем.

1. SQL-базы данных (SQLite)

Rusqlite: Позволяет запустить SQLite полностью в памяти. Достаточно указать путь :memory: при открытии соединения. Это дает поддержку SQL, транзакций и типизации.

2. Специализированные In-memory решения (Key-Value)

Если вам не нужен SQL, а достаточно хранилища «ключ-значение», подходят следующие библиотеки:

- PickleDB-rs: Максимально простая библиотека, вдохновленная Python. Хранит данные в памяти и может периодически сбрасывать их в файл.
- redb: Современная встраиваемая БД на чистом Rust. Она ориентирована на безопасность и производительность, поддерживает работу в памяти и имеет типизированный API.
- TinyDB / TinyBase: Компактные библиотеки для хранения сериализуемых структур данных в памяти с возможностью простой индексации.

3. Работа с многопоточностью

Если данные должны быть доступны из разных потоков:

- DashMap: Высокопроизводительная альтернатива стандартной HashMap, оптимизированная для параллельного доступа без необходимости блокировать всю таблицу целиком.
- Native DB: Современная база (актуальна на 2026 год), поддерживающая транзакции ACID, индексы и автоматическую миграцию моделей данных прямо в памяти.

4. Стандартная библиотека (самый простой путь)

Для простейших задач часто достаточно встроенных средств Rust. Если вам нужно просто хранить данные в памяти в рамках одного запуска программы без сложной логики запросов.
...
- Если нужны сложные запросы и SQL — используйте rusqlite с :memory:
- Если важна скорость и типизация — выбирайте redb
- Если нужно простое хранилище для настроек или небольших списков — PickleDB

# Тесты

## Автоматизация через Git Hooks

Чтобы тесты запускались автоматически перед каждым коммитом, используйте Git-хуки.

- Перейдите в папку .git/hooks/.
- Создайте файл pre-commit (без расширения).
- Добавьте туда скрипт:
...
cargo test
if [ $? -ne 0 ]; then
    echo "Ошибка: тесты провалены. Коммит отменен."
    exit 1
fi
```
...
Теперь вы не сможете сделать git commit, если тесты падают. 

## Настройка CI (GitHub Actions / GitLab CI)

Для серьезных проектов тесты всегда должны запускаться на сервере перед слиянием кода (Merge Request). В 2026 году это стандарт.
Пример для .github/workflows/rust.yml: 
